...
void PlayerHoldState::Enter(Player* player) {
	...
	if (CheckSoundMem(SoundManager::getInstance().checkID("motu.ogg")) != 1) {
...
void PlayerItemGetState::Enter(Player* player) {
	...
	if (CheckSoundMem(SoundManager::getInstance().checkID("sutokku.ogg")) != 1) {
		PlaySoundMem(SoundManager::getInstance().checkID("sutokku.ogg"), DX_PLAYTYPE_BACK);
	}
}
